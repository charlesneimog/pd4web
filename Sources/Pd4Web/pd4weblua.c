...
int sys_trytoopenone(const char *dir, const char *name, const char *ext, char *dirresult, char **nameresult,
...
static const char *pdlua_reader(lua_State *UNUSED(L), /**< Lua interpreter state. */
...
/** Proxy inlet 'anything' method. */
static void pdlua_proxyinlet_anything(t_pdlua_proxyinlet *p, /**< The proxy inlet that received the message. */
...
static void pdlua_proxyinlet_fwd(t_pdlua_proxyinlet *p, /**< The proxy inlet that received the message. */
...
/** Proxy inlet initialization. */
static void pdlua_proxyinlet_init(t_pdlua_proxyinlet *p, /**< The proxy inlet to initialize. */
...
/** Proxy receive 'anything' method. */
static void pdlua_proxyreceive_anything(t_pdlua_proxyreceive *r, /**< The proxy receive that received the message. */
                                        t_symbol *s,             /**< The message selector. */
                                        int argc,                /**< The message length. */
                                        t_atom *argv             /**< The atoms in the message. */
) {
...
/** Proxy receive allocation and initialization. */
static t_pdlua_proxyreceive *pdlua_proxyreceive_new(struct pdlua *owner, /**< The owning object. */
...
/** Proxy clock allocation and initialization. */
static t_pdlua_proxyclock *pdlua_proxyclock_new(struct pdlua *owner /**< The object to forward messages to. */
) {
...
/** Dump an array of atoms into a Lua table. */
static void pdlua_pushatomtable(int argc,    /**< The number of atoms in the array. */
...
/** Pd object constructor. */
static t_pdlua *pdlua_new(t_symbol *s, /**< The construction message selector. */
    ...
    if (needs_base) {
        ...
        if (fd >= 0) {
            ...
            pd_error(NULL, "lua: constructor: couldn't locate `%s'", buf);
        }
    }
...
#else
static void pdlua_motion(t_gobj *z, t_floatarg dx, t_floatarg dy, t_floatarg up)
#endif
{
#if !PLUGDATA
#ifndef PURR_DATA
    ...
    if (x->has_gui) {
        ...
        if (doit) {
            if (!x->gfx.mouse_down) {
                ...
                pdlua_gfx_mouse_drag(x, xpix, ypix);
            }
...
static void pdlua_dsp(t_pdlua *x, t_signal **sp) {
    ...
    x->sig_nchans = (t_int *)resizebytes(x->sig_nchans, x->sig_count * sizeof(t_int), sum * sizeof(t_int));
    x->sig_count = sum;
...
static int pdlua_get_arguments(lua_State *L) {
    ...
    if (lua_islightuserdata(L, 1)) {
        ...
        for (ap = binbuf_getvec(b); indx--; ap++, i++) {
            if (i == 0) {
                continue; // skip 1st atom = object name
            }
            lua_pushnumber(L, i);
...
static int pdlua_set_arguments(lua_State *L) {
    ...
    if (lua_islightuserdata(L, 1)) {
        ...
        if (lua_istable(L, 2)) {
            ...
            int redraw = !o->has_gui && gobj_shouldvis(&o->pd.te_g, o->canvas) && glist_isvisible(o->canvas);
 ...
 * */
{
    ...
    c = class_new(gensym((char *)name), (t_newmethod)pdlua_new, (t_method)pdlua_free, sizeof(t_pdlua),
    ...
    if (strcmp(name, "pdlua") && strcmp(name, "pdluax")) {
        ...
        c_gfx = class_new(gensym((char *)name_gfx), (t_newmethod)pdlua_new, (t_method)pdlua_free, sizeof(t_pdlua),
                          CLASS_NOINLET | CLASS_MULTICHANNEL, A_GIMME, 0);
...
static int pdlua_object_creategui(lua_State *L) {
    ...
    if (!o->pdlua_class_gfx) {
        return 0; // we're not supposed to be here...
    }
 ...
 * */
{
    ...
    if (lua_islightuserdata(L, 1)) {
        ...
        if (o) {
            ...
            int new_inlets = have_number ? luaL_checknumber(L, 2) : have_table ? lua_rawlen(L, 2) : 0;
            if (!have_number && !have_table) {
 ...
 * */
{
    ...
    if (lua_islightuserdata(L, 1)) {
        ...
        if (o) {
            ...
            int have_number = lua_isnumber(L, 2);
            int have_table = lua_istable(L, 2);
            int new_outlets = have_number ? luaL_checknumber(L, 2) : have_table ? lua_rawlen(L, 2) : 0;
            if (!have_number && !have_table) {
...
/** Dispatch Pd inlet messages to Lua objects. */
static void pdlua_dispatch(t_pdlua *o,         /**< The object that received the message. */
...
/** Dispatch Pd receive messages to Lua objects. */
static void pdlua_receivedispatch(t_pdlua_proxyreceive *r, /**< The proxy receive that received the message. */
                                  t_symbol *s,             /**< The message selector. */
                                  int argc,                /**< The message length. */
                                  t_atom *argv             /**< The atoms in the message. */
) {
...
/** Convert a Lua table into a Pd atom array. */
static t_atom *pdlua_popatomtable(lua_State *L, /**< Lua interpreter state.
                                                 * \par Inputs:
        ...
        if (o) {
            ...
            if (0 <= out && out < o->outlets) {
                if (lua_isstring(L, 3)) {
                    ...
                    if (s) {
                        if (strlen(s) != sl) {
                            pd_error(o, "%s: warning: selector symbol munged (contains \\0 in body) [outlet %d]",
                                     src_info(L, msg), out + 1);
                        }
                        lua_pushvalue(L, 4);
                        ...
                        if (count == 0 || atoms) {
                            ...
                            pd_error(o, "%s: error: %s atoms table [outlet %d]", src_info(L, msg),
                                     lua_isnoneornil(L, 4) ? "missing" : "invalid", out + 1);
                        }
                        if (atoms) {
                            free(atoms);
                            lua_pop(L, 4); /* pop all the arguments */
                        ...
                        pd_error(o, "%s: error: null selector [outlet %d]", src_info(L, msg), out + 1);
                    }
                } else {
                    pd_error(o, "%s: error: selector must be a string [outlet %d]", src_info(L, msg), out + 1);
                }
            } else {
                pd_error(o, "%s: error: outlet index out of range [outlet %d]", src_info(L, msg), out + 1);
            }
        } else {
            pd_error(NULL, "%s: error: null object for outlet", src_info(L, msg));
        }
    } else {
        pd_error(NULL, "%s: error: missing object for outlet", src_info(L, msg));
    }
    lua_pop(L, 4); /* pop all the arguments */
    ...
    if (lua_isstring(L, 1)) {
        ...
        if (receivesym) {
            if (strlen(receivename) != receivenamel) {
                pd_error(NULL, "%s: warning: receive symbol munged (contains \\0 in body) [send %s]", src_info(L, msg),
                         receivename);
            }
            if (lua_isstring(L, 2)) {
                ...
                if (selsym) {
                    if (strlen(selname) != selnamel) {
                        pd_error(NULL, "%s: warning: selector symbol munged (contains \\0 in body) [send %s]",
                                 src_info(L, msg), receivename);
                    }
                    lua_pushvalue(L, 3);
                    atoms = pdlua_popatomtable(L, &count, NULL);
                    if ((count == 0 || atoms) && (receivesym->s_thing)) {
                        typedmess(receivesym->s_thing, selsym, count, atoms);
                    } else {
                        pd_error(NULL, "%s: error: %s atoms table [send %s]", src_info(L, msg),
                                 lua_isnoneornil(L, 3) ? "missing" : "invalid", receivename);
                    }
                    if (atoms) {
                        free(atoms);
                    ...
                    pd_error(NULL, "%s: error: null selector [send %s]", src_info(L, msg), receivename);
                }
            } else {
                pd_error(NULL, "%s: error: selector must be a string [send %s]", src_info(L, msg), receivename);
            }
        } else {
            pd_error(NULL, "%s: error: null receive name in send", src_info(L, msg));
        }
    } else {
        pd_error(NULL, "%s: error: receive name in send must be string", src_info(L, msg));
    }
 ...
 * */
{
    ...
    if (lua_islightuserdata(L, 1)) {
        ...
        if (o) {
            ...
            pd_error(NULL, "lua: error: null object in error function");
        }
    } else {
        pd_error(NULL, "lua: error: bad arguments to error function");
    }
...
static void pdlua_setrequirepath(/* FIXME: documentation (is this of any use at all?) */
...
static void pdlua_clearrequirepath(/* FIXME: documentation (is this of any use at all?) */
                                   lua_State *L) {
 ...
 * */
{
    ...
    if (lua_islightuserdata(L, 1)) {
        ...
        if (c) {
            ...
            fd = trytoopenone(path && *path ? path : pdlua_datadir, filename, "", buf, &ptr, MAXPDSTRING, 1);
            if (fd >= 0) {
                ...
                pd_error(NULL, "lua: dofilex: couldn't locate `%s'", filename);
            }
        } else {
            pd_error(NULL, "lua: dofilex: null class");
        }
    } else {
        pd_error(NULL, "lua: dofilex: wrong type of object");
    }
    ...
    if (lua_islightuserdata(L, 1)) {
        ...
        if (o) {
            ...
            if (fd >= 0) {
                ...
                pd_error(o, "lua: dofile: couldn't locate `%s'", filename);
            }
        } else {
            pd_error(NULL, "lua: dofile: null object");
        }
    } else {
        pd_error(NULL, "lua: dofile: wrong type of object");
    }
    lua_pushstring(L, buf); /* return the path as well so we can open it later with pdlua_menu_open() */
...
static int pdlua_signal_setmultiout(lua_State *L) {
    ...
    if (g_signal_setmultiout) {
        if (x->sp && x->sp[x->siginlets + outidx]) {
            ...
            pd_error(x, "%s: signal_setmultiout: invalid signal pointer. must be called from dsp method",
                     src_info(L, msg));
            return 0;
        }
    } else {
...
/** Pd loader hook for loading and executing Lua scripts. */
static int pdlua_loader_fromfd(int fd,            /**< file-descriptor of .pd_lua file */
...
static int pdlua_loader_wrappath(int fd,            /**< file-descriptor of .pd_lua file */
                                 const char *name,  /**< The name of the script (without .pd_lua extension). */
                                 const char *dirbuf /**< The name of the directory the .pd_lua files lives in */
) {
    int result = 0;
...
static int pdlua_loader_legacy(t_canvas *canvas, /**< Pd canvas to use to find the script. */
...
static int pdlua_loader_pathwise(t_canvas *UNUSED(canvas), /**< Pd canvas to use to find the script. */
    ...
    if ((fd = trytoopenone(path, objectname, ".pd_lua", dirbuf, &ptr, MAXPDSTRING, 1)) >= 0) {
    ...
    if ((fd = trytoopenone(path, filename, ".pd_lua", dirbuf, &ptr, MAXPDSTRING, 1)) >= 0) {
        if (pdlua_loader_wrappath(fd, objectname, dirbuf)) {
            return 1;
        }
    }
...
#define BUILD_DATE __DATE__ " " __TIME__
    ...
    snprintf(pdluaver, MAXPDSTRING - 1, "pdlua %s (GPL) 2008 Claude Heiland-Allen, 2014 Martin Peach et al.",
             pdlua_version);
    snprintf(compiled, MAXPDSTRING - 1, "pdlua: compiled for pd-%d.%d on %s", PD_MAJOR_VERSION, PD_MINOR_VERSION,
             BUILD_DATE);
...
// compatibility handling copied from
// https://github.com/Spacechild1/vstplugin/blob/3f0ed8a800ea238bf204a2ead940b2d1324ac909/pd/src/vstplugin~.cpp#L4122-L4136
#ifdef _WIN32
    ...
    if (GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
        ...
        g_signal_setmultiout = (t_signal_setmultiout_fn)(void *)GetProcAddress(module, "signal_setmultiout");
    ...
    g_signal_setmultiout = (t_signal_setmultiout_fn)dlsym(dlopen(NULL, RTLD_NOW), "signal_setmultiout");
    g_glist_getrtext = (t_glist_rtext_fn)dlsym(dlopen(NULL, RTLD_NOW), "glist_getrtext");
    if (!g_glist_getrtext) {
        g_glist_getrtext = (t_glist_rtext_fn)dlsym(dlopen(NULL, RTLD_NOW), "glist_findrtext");
    }
#endif
    ...
    /*    fd = canvas_open(canvas_getcurrent(), "pd", ".lua", buf, &ptr, MAXPDSTRING, 1);  looks all over and rarely
     * succeeds */
