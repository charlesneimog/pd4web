...
typedef struct _pdlua_gfx {
    ...
    char
        current_item_tag[64]; // Tcl/tk tag that is only attached to the current drawing in progress
    char **layer_tags;
    int num_layers;
...
#else
    void (*plugdata_draw_callback)(void *, int, t_symbol *, int,
                                   t_atom *); // Callback to perform drawing in plugdata
#endif
} t_pdlua_gfx;
