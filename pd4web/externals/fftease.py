...
def fftease_extra(librarySelf: PureDataExternals):
    ...
    for root, _, files in os.walk(folder):
        for file in files:
            for required in ["fftease.h", "fftease_utilities.c", "fold.c", "fft4.c", "overlapadd.c", "makewindows.c"]:
                if file == required:
                    shutil.copy(os.path.join(root, file), os.path.join(librarySelf.PROJECT_ROOT, "webpatch", "externals", file))
